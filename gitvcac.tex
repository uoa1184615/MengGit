...
\begin{github}
I tentatively suggest using GitHub for the Mac (requires OS~10.6 or later), but use another \textsc{gui} if you prefer.\footnote{Another \textsc{gui} is git gui which comes with git; type the command \texttt{git~gui} in a terminal window.}
...
\item Checking and downloading, `pulling', any changes by your collaborators is one additional step you might do before starting work each session: from menu \texttt{Repository} select \texttt{Pull}; alternatively, from the very right of the top menu bar, click on \texttt{Branch in Sync}.
\end{itemize}
\end{github}
