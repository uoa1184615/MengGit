...
\end{gitgui}
\begin{github}
To share and collaborate we need an information store on the web.  The company GiHub provides some free storage (as well as a commercial service).  To \emph{start}, let's use GitHub.

\paragraph{Create an account} Go to \url{https://github.com/plans} and click on \texttt{Create a free account}.   Follow the instructions which involves: registering; generating an ssh key to give to GitHub; save in your git preference the \textsc{api}-token from GitHub; and setting git config.  GitHub's instructions lead you through the process. 
\end{github}


\subsection{Clone an existing project}
\begin{github}
\begin{itemize}
\item Use a web browser to connect to GitHub \url{https://github.com}, and login (top-right).
\item Find the project (somehow): for example, just go to \url{https://github.com/uoa1184615/MengGit}
\item On the left-side beneath  ``uoa1184615/MengGit '' and beneath ``Code'', you should see and click on \texttt{Clone in Mac}.
...
\item Drag that new folder onto GitHub for GitHub to manage the local repository.  
\item Thereafter, work and commit as in Sections~\ref{sec:wiss}--\ref{sec:ryw}, but additionally occasionally \emph{Click the \texttt{Sync} button on the \texttt{Changes} window}.\footnote{I believe the \texttt{Sync} operation not only copies information to the remote repository, but also merges into your local repository any changes made there by collaborators in parallel to you.}
\end{itemize}
\end{github}
