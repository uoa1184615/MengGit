...
Assume~$\nu=0$, thus there is no bed drag term in the \textsc{pde}s. Seek solutions in exponential form
\begin{eqnarray}
...
\end{eqnarray}
where $\xi=(x-X_j)/D$ such that~$\partial_x=\frac1D\partial_\xi$, and~$k$ is the lateral wavenumber.
Substituting these exponential forms into the \textsc{pde}s gives
\begin{equation}
\lambda^2u(\xi)=\frac{1}{D^2}\frac{\partial^2u(\xi)}{\partial\xi^2}\,,\quad\text{and}\quad\lambda^2u'(\xi)=\frac{1}{D^2}\frac{\partial^2u'(\xi)}{\partial\xi^2}\,,
\end{equation}
which indicate solutions in the forms of 
\begin{eqnarray}
&&u(\xi)=A\cos\ell\xi+B\sin\ell\xi\,,\label{sol:uxi}\\
&&u'(\xi)=A'\cos\ell\xi+B'\sin\ell\xi\,,\label{sol:u'xi}
\end{eqnarray}
where~$\ell$ is the wavenumber of subgrid structures.
Take the above~$u(\xi)$ and~$u'(\xi)$ into the governing \textsc{pde}s again and obtain the solutions of~$h(\xi)$ and~$h'(\xi)$ in the forms of 
\begin{eqnarray}
&&h'(\xi)=\frac{\ell}{D\lambda}(A\sin\ell\xi-B\cos\ell\xi)\,,\label{sol:h'xi}\\
&&h(\xi)=\frac{\ell}{D\lambda}(A'\sin\ell\xi-B'\cos\ell\xi)\,,\label{sol:hxi}
\end{eqnarray}

The coupling conditions proposed in the introduction indicate
\begin{align}&
...
\end{align}
Substitute the solutions forms of~$h(\xi)$,~$u(\xi)$,~$h'(\xi)$ and~$u'(\xi)$ into the above coupling conditions, rearrange and obtain
\begin{eqnarray*}
...
\end{eqnarray*}
The determinant of the coefficient matrix equaling to zero gives
\begin{equation}
...
Two cases arise. 
\begin{enumerate}
\item If~$\cos\ell=0$, then~$\ell=\pi/2,3\pi/2,5\pi/2,\ldots$, which demonstrate the subgrid structures are good.
\item If~$(2-\gamma)\sin\ell+\gamma\sin\ell\cos2k\pm\gamma\sin2k=0$, when~$\gamma=0$, obtain~$\sin\ell=0$, which indicates~$\ell=0,\pi,2\pi,\ldots$. When~$\gamma=1$, obtain 
  \[
